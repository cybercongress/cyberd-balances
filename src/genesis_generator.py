...
def load_json(emission, manual_json, part_percentage):
...
def load_csv(emission, path, part_percentage):
...
def get_distributions(emission, distribution_json, manual_json):
    ...
    for distribution_type in CSV_DISTRIBUTIONS:
        ...
        balances_df = load_csv(emission, CSV_DISTRIBUTIONS[distribution_type], part_percentage)
    ...
    for distribution_type in distribution_json.keys():
        if distribution_type not in CSV_DISTRIBUTIONS:
            part_percentage = int(distribution_json[distribution_type]) / 100
            balances_df = load_json(emission, manual_json[distribution_type], part_percentage)
            all_balances.append(balances_df)
...
def save_json(emission, genesis_json, balances):
    ...
    assert emission - balances.sum() <= 1000
